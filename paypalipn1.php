...
// use the above params to look up what the price of "item_name" should be.
		//mail('tanmoy@scriptgiant.com',$sub);
	//$amount_they_should_have_paid = $payment_amount;
...
// you must check at the least the following...
      ...
      if (($payment_status == 'Completed') &&  //check they payed what they should have
...
if(!$row)
{
			...
			//mail('tanmoy@scriptgiant.com',);

			/*$sql = "SELECT * FROM customers WHERE payment_status = 'Paid' AND paypal_transaction_id='$txn_id' AND id = '$order_id' ";
			$res = mysql_query ($sql);
			$row=mysql_fetch_array($res);*/
			...
			//$download_url_encrypted = "http://consultsunlimitedinc.com/download_purchased.php?did=".strrev(md5($row[product_id]));
...
//        uncomment this section during development to receive an email to indicate whats happened
           /*$mail_To = "abhik_stfc@hotmail.com";
...
/*
$mail_To = "ipeg.solutions@gmail.com";
$mail_Subject = "PayPal  IPN  Called";
$mail_Body = "Paypal just sent some ipn stuff";
mail($mail_To, $mail_Subject, $mail_Body);
*/
?>