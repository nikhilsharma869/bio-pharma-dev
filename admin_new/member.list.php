...
<?
	...
	if($_REQUEST[param] && $_REQUEST[search]):
		...
		if(isset($_REQUEST[alpha]) && $_REQUEST[alpha] != '' && $_REQUEST[status]!='') {
		$cond = "fname rlike '".$_REQUEST[alpha]."'";
		$parama = "&status=".$_GET['status']."&alpha=".$_REQUEST[alpha];
	}
	...
	if($_REQUEST['gold_member'])
	{
	 ...
	 if(isset($_REQUEST['status']) && isset($_REQUEST[alpha]))
	{
		$cond="status='".$_REQUEST['status']."' AND fname rlike '".$_REQUEST[alpha]."'";
		$parama = "&status=".$_GET['status']."&alpha=".$_REQUEST[alpha];
	} 
...
?>
<strong>Browse By First Name</strong>: <a href='<?=$_SERVER['PHP_SELF'].$st?>' class="lnk" <?php if($_GET['alpha']=='') { ?>style="color: #000; font-weight: bold;" <?php } ?>>All</a>&nbsp;|&nbsp;
<?php
	for($i=65;$i<=90;$i++)
	{
	 $st = '';
	...
	?>
	<a href="<?=$_SERVER['PHP_SELF']?>?alpha=<?=chr($i).$st?>" class='lnk' <?php if($_GET['alpha']==chr($i)) { ?>style="color: #000; font-weight: bold;" <?php } ?> ><?=chr($i)?></a>
	<?php
		if($i!=100)
		{
			echo"&nbsp;|&nbsp;";
...
<?php
//echo "select * from  ".$prev."user ".$cond2." order by user_id desc limit ".($page-1)*10 .", 10";  
...
while($d=@mysql_fetch_array($r)):
	...
	$wonbids = @mysql_num_rows(mysql_query("SELECT * FROM " . $prev . "projects WHERE chosen_id='" . $d[user_id] . "'"));
   $totalclosedproject=$closejobs+$closedjobs;
   if(!empty($d[logo]))
	{
	 $temp_logo=$d[logo];
	...
	<td>" .$balnc . "</td><td>		Open Project=>" .$openjobs."<br />
	Close Project=>". $totalclosedproject." <br />
	Cancel Project=>".$cancelledjobs ." <br />
	Won Jobs =>".$wonbids." <br />
	</td><td >" . $status . "<br>";
	
	echo"<br>Reviews :";
	echo"</td>";
	echo "<td align=center><a class='lnk  tip' title='Edit Member'  href='edit_member.php?user_id=$d[user_id]'><u>".$main_icon_edit_img."</u></a> | <a class=lnk  href='add_fund.php?user_id=$d[user_id]'><u>Add Fund</u></a> </td>";
	echo "</tr>";
    $j++;
endwhile;s
?></tbody></table>
