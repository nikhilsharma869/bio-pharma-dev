...
function responsive_slider() {
	...
	$slides = new WP_Query( array( 'post_type' => 'slides','orderby'=>'menu_order' , 'order' => 'ASC', 'posts_per_page' => '20' ) );
...
$slider .= get_the_post_thumbnail( $post->ID, 'full', array( 'class' => 'slide-thumbnail' ) );
		...
		$slider .= '<script>
					...
					var ah=h-102;
					jQuery(".responsive-slider").find(".slides").find("img").css("height",ah+"px")
					});
					</script>';
...
function responsive_slider_register_shortcode() {
	add_shortcode( 'wp_responsive_slider', 'responsive_slider' );
}
