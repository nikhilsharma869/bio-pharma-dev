...
class OptionsFramework_Backup {
	...
	function init () {
		if ( is_admin() && ( get_option( 'framework_woo_backupmenu_disable' ) != 'true' ) ) {
			...
			add_action('wp_ajax_widget_import_submit', array($myWidget_Data, 'widget_import_submit'));
		}		
	} // End init()
	...
	function register_admin_screen () {
		...
		$this->admin_page_import = add_submenu_page('options-framework', 'Import', 'Import', 'administrator', 'options-framework-import',  array(&$this, 'admin_screen_import'));
		$this->admin_page_export = add_submenu_page('options-framework', 'Export', 'Export', 'administrator', 'options-framework-export',  array(&$this, 'admin_screen_export'));
	...
	<div id="optionsframework-metabox" class="metabox-holder">
		<div id="optionsframework" class="postbox">
			<div class="wrap">
				...
				<h3><?php _e( 'Import', 'cherry' ); ?></h3>
				
				<div class="import">
					<?php $step = empty( $_GET['step'] ) ? 0 : (int) $_GET['step'];
				...
				<div class="progress-holder">
					<div class="progress progress-striped">
						...
						<div class="circle finish"><span>Done</span></div>
					</div>
				</div><!-- /.progress-holder -->
			</div><!--/.wrap-->
		</div>
	...
	function callImportTheme () { ?>
		<div class="import-options-settings">
			<h4 class="head"><?php _e( '<span>Step <i>1</i></span>Theme Options Import', 'cherry' ); ?></h4>			
			<p class="text-style"><?php _e( 'Select the file that contains Theme Options (e.g. <b>options.json</b> )', 'cherry' ); ?></p>
			...
			<form enctype="multipart/form-data" method="post" action="<?php echo admin_url( 'admin.php?page=options-framework-import'); ?>" id="optionsForm">
				...
				<input type="submit" class="button-primary" value="<?php _e( 'Upload File and Import', 'cherry' ); ?>" disabled="disabled">
			</form>
		</div>
	<?php 
	}
...
<?php
	...
	function callExportTheme () { ?>
		<h4 class="head"><?php _e( '<span>Step <i>1</i></span>Theme Options Export', 'cherry' ); ?></h4>
		...
		<form method="post" action="" id="theme-options-export">
			...
			<input type="submit" class="button-primary fnone" value="<?php _e( 'Next &raquo;', 'cherry' ); ?>" />
		</form>
	<?php }
	...
	/*function admin_screen_help ( $contextual_help, $screen_id, $screen ) {
		...
		$contextual_help =
		  '<h3>' . __( 'Welcome to the OptionsFramework Backup Manager.', 'cherry' ) . '</h3>';		
		} // End IF Statement
	...
	*/
	...
	function admin_notices () {
		...
		/*echo '<div id="import-notice" class="updated"><p>' . sprintf( __( 'Please note that this backup manager backs up only your Theme Options and not your content. To backup your content, please use the %sWordPress Export Tool%s.', 'OptionsFramework' ), '<a href="' . admin_url( 'export.php' ) . '">', '</a>' ) . '</p></div><!--/#import-notice .message-->' . "\n";*/
