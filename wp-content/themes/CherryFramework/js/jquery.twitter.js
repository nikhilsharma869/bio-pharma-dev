(function($) {
	...
	*/
	...
	$.fn.getTwitter = function(options) {
		...
		$(this).append('<ul class="twitter_list unstyled"><li class="twitter_li"></li></ul>');
		...
		if (o.showProfileLink) {
			$(this).append('<a class="twitter_profileLink" href="http://twitter.com/'+o.userName+'"><span class="inner">Follow Us</span></a>');
		}
		...
		function twitterCallback2(twitters) {
		  ...
		  for (var i=0; i<twitters.length; i++){
			...
			statusHTML.push('<li class="twitter_li"><div class="twitter_txt">'+status+'</div><a class="twitter_time" href="http://twitter.com/'+username+'/statuses/'+twitters[i].id_str+'">'+relative_time(twitters[i].created_at)+'</a></li>');
		  }
		  var id = '';
		  id = o.id;
		...
		$.getJSON("https://api.twitter.com/1/statuses/user_timeline.json?screen_name="+o.userName+"&callback=?&count="+o.numTweets, function(data) {twitterCallback2(data);
			  ...
			  // remove preLoader from container element
				...
				$(".twitter_list li:first").addClass("firstTweet");
	
				// give last list item a special class
				$(".twitter_list li:last").addClass("lastTweet");
