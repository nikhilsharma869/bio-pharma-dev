...
<p><a href="<?=$vpath?>"><?=$lang['HOME_LINK']?></a> | <a href="javascript:void(0);" class="selected"><?=$lang['FROZEN_PROJECTS']?></a></p></div>
<div class="clear"></div>
...
$res21=mysql_query("SELECT * FROM " . $prev . "projects WHERE user_id='" . $_SESSION[user_id] . "' and status IN ('frozen') ORDER BY id,date2 DESC");
$total =@mysql_num_rows($res21);

$tinyres = mysql_query("SELECT * FROM " . $prev . "projects WHERE user_id='" . $_SESSION[user_id] . "' and status IN ('frozen') ORDER BY id,date2 DESC limit " . ($_REQUEST['page']-1)* $no_of_records. ",".$no_of_records."");
...
<?php 
	...
	<a href="'.$vpath.'my-jobs/pick/' . $kikrow[id] . '/" class=link_class><u>'.$lang['PICK_PROVIDER'].'</u></a> | <a href="'.$vpath.'extend_project/' . $kikrow[id] . '/" class=link_class><u>'.$lang['EXTEND'].'</u></a> |  <a href="'.$vpath.'my-jobs/close/' . $kikrow[id] . '/" class=link_class><u>'.$lang['CLOSE'].'</u></a><br>

	'.$lang['AWAIT_RESP'].' <i><a href="'.$vpath.'publicprofile/' . getusername($kikrow[chosen_id]) . '/" class=link_class><u>' . getusername($kikrow[chosen_id]) . '</u></a></i>)</td>';
