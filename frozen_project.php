    ...
    <div class="main_div2">
        <div class="inner-middle"> 
            ...
            <div class="profile_right">
					...
					$res21=mysql_query("SELECT * FROM " . $prev . "projects WHERE user_id='" . $_SESSION[user_id] . "' and (status='frozen') ORDER BY id,date2 DESC");
					$total =@mysql_num_rows($res21);

					$tinyres = mysql_query("SELECT * FROM " . $prev . "projects WHERE user_id='" . $_SESSION[user_id] . "' and (status='frozen' ) ORDER BY id,date2 DESC limit " . ($_REQUEST['page']-1)* $no_of_records. ",".$no_of_records."");
				   ...
				   <div class="search-job-content clearfix">
						...
						<div class="joblist_button_group">
							...
							echo' <img src="images/register_icon.png"><a href="'.$vpath.'my-jobs/pick/' . $kikrow[id] . '/" class=link_class><u>'.$lang['PICK_PROVIDER'].'</u></a> 
									&nbsp;|&nbsp; <img src="images/extend.png"><a href="'.$vpath.'extend_project/' . $kikrow[id] . '/" class=link_class><u>'.$lang['EXTEND'].'</u></a> 
									&nbsp;|&nbsp; <img src="images/clo.png"> <a href="'.$vpath.'my-jobs/close/' . $kikrow[id] . '/" class=link_class><u>'.$lang['CLOSE'].'</u></a><br>
	'.$lang['AWAIT_RESP'].' <i>
	<a href="'.$vpath.'publicprofile/' . getusername($kikrow[chosen_id]) . '/" class=link_class><u>' . getusername($kikrow[chosen_id]) . '</u></a></i>)';
							?>
						</div>
