...
if($_SESSION[lang_id]){
	...
	$row_content=mysql_fetch_array(mysql_query("select * from ".$prev."contents where cont_title='User agreement'"));
}
...
$r = mysql_query("SELECT * FROM ".$prev."testimonial WHERE status= 'Y' ORDER BY post_date ");
while(@$d=mysql_fetch_assoc($r)){
?>
   <!--Howitworks Start-->
    
    	<div class="testimonials_sec" style="width:900px; text-align:left">
  <img src="<?=$vpath.$d['picture']?>" />
  <div class="testi_text"> &nbsp; &nbsp;
<span >"</span>  <?=$d['comment']?> <span >"</span>
