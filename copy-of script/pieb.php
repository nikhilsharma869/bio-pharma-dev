...
$query=mysql_query("select * from ".$prev."projects p inner join ".$prev."buyer_bids b on p.id=b.project_id where b.bidder_id='".$_SESSION['user_id']."' group by b.project_id");
$bid_query=mysql_query("select * from ".$prev."projects p inner join ".$prev."buyer_bids b on p.id=b.project_id where b.bidder_id='".$_SESSION['user_id']."' and p.status='open' group by b.project_id");
$process_query=mysql_query("select * from ".$prev."projects p inner join ".$prev."buyer_bids b on p.id=b.project_id where b.bidder_id='".$_SESSION['user_id']."' and p.status='process' group by b.project_id");
//echo "select * from ".$prev."projects p inner join ".$prev."buyer_bids b on p.id=b.project_id where b.bidder_id='".$_SESSION['user_id']."' and p.status='complete' group by b.project_id";
$complete_query=mysql_query("select * from ".$prev."projects p inner join ".$prev."buyer_bids b on p.id=b.project_id where b.bidder_id='".$_SESSION['user_id']."' and p.status='complete' group by b.project_id");

 $query1=mysql_num_rows($query);
$bid_query1=mysql_num_rows($bid_query);
$process_query1=mysql_num_rows($process_query);
$complete_query1=mysql_num_rows($complete_query);

$total=$query1;
$bid=$bid_query1;
$process=$process_query1;
$complete=$complete_query1;

 $data = $process."*".$complete."*".$bid;
$label = "Process*Complete*No result";
//$data = $_GET["data"];
//$label = $_GET["label"];
