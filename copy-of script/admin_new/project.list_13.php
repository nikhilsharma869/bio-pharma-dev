...
$r=mysql_query("select *  from " . $prev . "projects," .$prev ."projects_cats where ". $prev ."projects.id=" .$prev ."projects_cats.id and ". $prev ."projects.status='process' group by ".$prev ."projects.id");
...
$r=mysql_query("select * from " . $prev . "projects," .$prev ."projects_cats where ". $prev ."projects.id=" .$prev ."projects_cats.id and ". $prev ."projects.status='close' group by ".$prev ."projects.id");
...
?>
<table  border="0" width="100%"  cellpadding="4" cellspacing="1">
<tr class=link><td style="border:solid 1px gray;padding:5px" <?if($_REQUEST[creation]){echo"bgcolor='#e5e5e5'";}?>><a href='<?=$_SERVER[PHP_SELF]?>?creation=<?=date("Y-m-d")?>' class=lnk>Today's Post : <?=$total?></a></td>
<td style="border:solid 1px gray;padding:5px" <?if($_REQUEST[status]=="open"){echo"bgcolor='#e5e5e5'";}?>><a href='<?=$_SERVER[PHP_SELF]?>?status=open' class=lnk>Open Projects : <?=$total_open?></a></td>
<td style="border:solid 1px gray;padding:5px" <?if($_REQUEST[status]=="process"){echo"bgcolor='#e5e5e5'";}?>><a href='<?=$_SERVER[PHP_SELF]?>?status=process' class=lnk>Running Projects : <?=$total_process?></a></td>
<td style="border:solid 1px gray;padding:5px" <?if($_REQUEST[status]=="complete"){echo"bgcolor='#e5e5e5'";}?>><a href='<?=$_SERVER[PHP_SELF]?>?status=complete' class=lnk>Completed Projects : <?=$total_delevered?></a></td>
<td style="border:solid 1px gray;padding:5px" <?if($_REQUEST[status]=="expired"){echo"bgcolor='#e5e5e5'";}?>><a href='<?=$_SERVER[PHP_SELF]?>?status=expired' class=lnk>Expired : <?=$total_expired?></a></td>
<td style="border:solid 1px gray;padding:5px" <?if($_REQUEST[status]=="close"){echo"bgcolor='#e5e5e5'";}?>><a href='<?=$_SERVER[PHP_SELF]?>?status=close' class=lnk>Closed Projects : <?=$total_cancelled?></a></td>
<td style="border:solid 1px gray;padding:5px" <?if($_REQUEST[status]=="frozen"){echo"bgcolor='#e5e5e5'";}?>><a href='<?=$_SERVER[PHP_SELF]?>?status=frozen' class=lnk>Frozen Projects : <?=$total_frozen?></a></td>
</tr></table>
<br>
...
if($_POST[param]=='username'){
  ...
  $cond[]=$prev . "projects.user_id = '" . $u[user_id] . "'";
  }else{
