...
if (isset($_POST['emp_reg'])) {
    ...
    if (!$emailerror) {
        ...
        if (!$emailerror && (@mysql_num_rows(mysql_query("select user_id from " . $prev . "user where email like '%" . txt_value($_REQUEST['email']) . "%'")))) {
    ...
    if ($errormsg == 'false' || $_SESSION['error'] == "") {
        ...
        $planqrry = mysql_query("SELECT * FROM `" . $prev . "membership_plan` WHERE `plane_id`='1'");
        ...
        mysql_query("INSERT INTO `" . $prev . "usermembership` SET 
                ...
                `sub_date`=NOW(),
                `exp_date`=DATE_ADD(NOW(), INTERVAL 7 DAY) ");
        if ($_POST['newsletter']) {
        ...
        if ($user_id) {
            ...
            $url = "<a href='" . $vpath . "activate/?v_key=" . md5($_REQUEST['username']) . "&user=" . ($user_id) . "'>";
