...
<?
		if($_REQUEST[sub_cat_id]){
			 ...
			 $cat=$_GET['sub_cat_id']."/".$cat_parent."/".replacename($cat_name2)."/";
			}else{
			 $cat="0/0/All/";
			}
			if($_GET['project_type']!=''){
  ...
  <div class="subdcribe-bar-right"></div>
   ...
   if($total_pages > 0)
	...
	//////////////////////////////////////////// select bids //////////////////////////////////////////////////	
			...
			$rr=mysql_query("select * from " . $prev . "categories," . $prev . "projects_cats where " . $prev . "categories.cat_id=" . $prev . "projects_cats.cat_id and " . $prev . "projects_cats.id=" . $row[id]." limit 3");
...
$mncat[]=$dd['parent_id'];			  
			  $txt.= "<a href='javascript:void(0)'>".$dd[cat_name] . "</a>  ";
			}
			$maincat="";
if(count($mncat)>0){
$rtu=implode(",",$mncat);
