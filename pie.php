...
$query=mysql_fetch_array(mysql_query("select count(*) as cnt from ".$prev."projects where user_id='".$_SESSION['user_id']."'"));
//echo "select * from ".$prev."projects p inner join ".$prev."buyer_bids b on p.id=b.project_id where p.user_id='".$_SESSION['user_id']."' and p.status='open' group by b.project_id";
$bid_query=mysql_query("select * from ".$prev."projects p inner join ".$prev."buyer_bids b on p.id=b.project_id where p.user_id='".$_SESSION['user_id']."' and p.status='open' group by b.project_id");

$open_query=mysql_fetch_array(mysql_query("select count(*) as cnt from ".$prev."projects where user_id='".$_SESSION['user_id']."' and status='open'"));
$frozen_query=mysql_fetch_array(mysql_query("select count(*) as cnt from ".$prev."projects where user_id='".$_SESSION['user_id']."' and status='frozen'"));
$process_query=mysql_fetch_array(mysql_query("select count(*) as cnt from ".$prev."projects where user_id='".$_SESSION['user_id']."' and status='process'"));
$complete_query=mysql_fetch_array(mysql_query("select count(*) as cnt from ".$prev."projects where user_id='".$_SESSION['user_id']."' and status='complete'"));

$bid_query1=mysql_num_rows($bid_query);
$total=$query['cnt'];
$bid=$bid_query1;
$open=$open_query['cnt'];
$frozen=$frozen_query['cnt'];
$process=$process_query['cnt'];
$complete=$complete_query['cnt'];

$data = $process."*".$frozen."*".$complete."*".$open;
$label = "Process*Frozen*Completed*Open";
//$data = $_GET["data"];
//$label = $_GET["label"];
